function [slphantom] = construct_shepp_logan(pixelsize)
%slphantom = construct_shepp_logan(pixelsize)
%CONSTRUCT_SHEPP_LOGAN Summary of this function goes here
%code is devloping data for shepp and logan phanton
% n = size of shepp logan (n*n)
x= -1:2/(pixelsize-1):1;
y=1:-2/(pixelsize-1):-1;
[X,Y]= meshgrid(x,y);
[x0,y0,A,B,alpha,rho] = load_shepplogan_data();
 slphantom = zeros(pixelsize,pixelsize);
 for k=1:length(x0);
  
  check1 = find(   (( ( ( X-x0(k) )*cosd(alpha(k)) + ( Y-y0(k) )*sind(alpha(k)) ).^2) / A(k)^2 ) + ( ( ( (X-x0(k) )*sind(alpha(k)) - ( Y-y0(k) )*cosd(alpha(k)) ).^2) / B(k).^2 ) <= 1 );
    slphantom(check1) = slphantom(check1) + rho(k); 
    
end

